...
TwinMind 整合了**卡片盒筆記法 (Zettelkasten)**、**LYT (Linking Your Thinking)** 與 **PARA 方法**，透過 AI CLI 的 Skills 與 Hooks 機制，讓 AI 全權處理知識庫的建卡、連結、維護與專案管理。支援 **Claude Code** 與 **Gemini CLI** 雙平台，共用同一個 vault。你只需要用自然語言說話，AI 就是你的第二大腦。
...
TwinMind 的所有操作都透過自然語言完成。你不需要學習任何指令 — 只需要像跟人說話一樣告訴 AI 你的想法，系統會自動判斷意圖並執行對應操作。
...
- **Link Inference**：建立卡片後，AI 自動分析既有卡片並建立相關連結
- **Post-op Pipeline**：每次狀態變更後，自動更新 Changelog、MOC、Home 與 Dashboard
...
- [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) 或 [Gemini CLI](https://github.com/google-gemini/gemini-cli) — 主要互動介面（擇一即可）
...
啟動後 AI 會自動讀取索引與設定，完成初始化後你就可以開始用自然語言管理你的知識庫了。
...
```text
TwinMind/
├── CLAUDE.md                 # Claude Code 操作指南與意圖路由
├── GEMINI.md                 # Gemini CLI 操作指南與意圖路由
├── .claude/
│   ├── skills/               # 12 個模組化 Skill（知識、行動、維護）
│   ├── hooks/                # 5 個驗證 Hook（自動確保資料一致性）
│   └── settings.json         # Hook 設定
├── .gemini/
│   ├── skills/               # 同上（適配 Gemini CLI tool 名稱）
│   ├── hooks/                # 同上（適配 AfterTool event）
│   ├── agents/               # Subagent 定義（post-op）
│   └── settings.json         # Hook 設定
├── docs/
│   └── agent-development.md  # 雙平台同步維護指南
└── vault/                    # 知識庫本體（用 Obsidian 開啟此資料夾）
    ...
    └── System/               # 系統設定與索引
        ...
        ├── changelog.md      # 操作日誌索引頁
        └── changelog-YYYY-MM.md  # 月度操作日誌（自動建立）
```
...
兩個頁面都**不需要手動維護**。每次知識庫或行動層發生狀態變更後，Post-op Pipeline 會自動重新生成對應頁面（Claude Code 在背景執行，Gemini CLI 同步執行）：

...
由 `twinmind:project` skill 引用。包含所有專案操作的詳細步驟。
...
4. **反思鉤（Reflection Hook）**：
   ...
   - 使用者回答 → 調用 `twinmind:capture` 建卡，`source` 設為 `"PARA/Projects/<name>/goal.md"`
...
6. **反思鉤（Reflection Hook）**：
   ...
   - 使用者回答 → 調用 `twinmind:capture` 建卡，`source` 設為 `"PARA/Projects/<name>/actions.md"`
   - 使用者拒絕 → 跳過

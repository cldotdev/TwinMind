...
關係類型、分類規則、Connections 格式和反向對照表的完整定義，請讀取 `.gemini/skills/tm-capture/references/link-inference.md`（與 capture 共用同一套連結系統）。

一致性驗證由 AfterTool hooks 自動處理。完成操作後啟動 subagent 執行 post-op pipeline（`layer: "knowledge"`）。

### Post-op Subagent 啟動方式

透過 subagent 啟動同步 subagent，prompt 包含 post-op payload：
...
5. 執行連結建立程序（見 `references/link-inference.md` 的「建立連結程序」）：
   ...
   - 寫入目標卡片反向連結（依反向對照表）
   - 更新 `vault-index.json`（**單次 `replace`**）：old_string 涵蓋源 note entry、目標 note entry 和 stats 物件的連續 JSON 區塊；new_string 包含完整更新版本（links_to/linked_from 加入對應 ID、重算兩方 link_count、重算 stats.total_links、更新 stats.last_updated）。此操作 SHALL 為單次 `replace` tool invocation
6. 啟動 post-op subagent（event_type: `LINK_CREATED`）
...
4. 執行移除：
   ...
   - 更新 `vault-index.json`（**單次 `replace`**）：old_string 涵蓋源 note entry、目標 note entry 和 stats 物件的連續 JSON 區塊；new_string 包含移除連結後的完整版本（links_to/linked_from 移除對應 ID、重算兩方 link_count、重算 stats.total_links、更新 stats.last_updated）。此操作 SHALL 為單次 `replace` tool invocation
5. 啟動 post-op subagent（event_type: `LINK_REMOVED`）

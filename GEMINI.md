...
啟動完成後，所有使用者輸入皆經由「意圖解析」層路由。非知識庫相關輸入（如程式碼問題、閒聊）則 bypass 意圖路由，以一般 Gemini CLI 模式回應。

**Step 4 — 衍生檔案新鮮度檢查**

比較 `vault-index.json` 的 `stats.last_updated` 與 `vault/Home.md` 及 `vault/PARA/Dashboard.md` 的檔案修改時間。若任一衍生檔案的修改時間落後 `last_updated` 超過 60 秒，啟動 post-op subagent 執行補償性 post-op：

- Home.md 過期 → `layer: "knowledge"`
- Dashboard.md 過期 → `layer: "action"`
- 兩者都過期 → `layer: "both"`

此檢查用於偵測前次 session 的 post-op 是否未完成（如 subagent timeout 或錯誤）。若兩者都在 60 秒內，不觸發任何補償。
...
當輸入與知識庫操作無關時（如閒聊、程式碼問題、系統指令），AI 不進入意圖路由，以一般 Gemini CLI 模式回應。
...
分類意圖後，AI 使用 activate_skill 調用對應的 `tm-*` skill。**自動 dispatch——不詢問使用者要調用哪個 skill。**

| 意圖 | Skill | post-op layer | Post-op 執行模式 | Link Inference 模式 |
|------|-------|---------------|-----------------|-------------------|
| `CAPTURE` | `tm-capture` | `knowledge` | Foreground subagent | Foreground subagent |
| `INBOX` | `tm-inbox` | `action`（升格為 Card 時用 `both`） | Foreground subagent | Foreground subagent（升格為 Card 時） |
| `ACTION` | `tm-action` | `action` | Foreground subagent | — |
| `TASK` | `tm-task` | `action` | Foreground subagent | — |
| `PROJECT` | `tm-project` | `action` | Foreground subagent | — |
| `AREA` | `tm-area` | `action` | Foreground subagent | — |
| `QUERY` | `tm-query` | 不需要（純唯讀） | — | — |
| `REVIEW` | `tm-review` | 僅索引修復/重建後（`both`） | Foreground subagent | — |
| `CONNECT` | `tm-connect` | `knowledge` | Foreground subagent | — |

### Post-op 規則

狀態變更操作完成後，對應 skill 透過 subagent 調用 post-op pipeline。Subagent 以同步（foreground）方式執行，main agent 等待 post-op 完成後再回應使用者。
...
- **Post-op subagent**：以同步方式啟動 `post-op` subagent（定義於 `.gemini/agents/post-op.md`）。Main agent 等待 subagent 完成後再回應使用者。適用於 changelog、MOC、Home.md、Dashboard.md 等確定性重建操作。
- **Link inference subagent**：以同步方式啟動 `link-inference` subagent（定義於 `.gemini/agents/link-inference.md`）。Main agent 等待 subagent 回傳建議清單後，根據使用者上下文決定採納哪些連結。
...
3. Main agent 啟動 post-op subagent（同步等待完成）
4. Subagent 完成後，main agent 回應使用者

**Post-op subagent 不寫入 vault-index.json。** MOC 變更透過 return message 回報。

#### 錯誤處理

- **Post-op subagent 失敗**：subagent 自行讀取 hook error 並重試一次。重試仍失敗則回傳失敗訊息，main agent 告知使用者。
- **Link inference subagent 失敗**：main agent 跳過 link inference，通知使用者自動連結建議暫時不可用。

### Hook 自動驗證

AfterTool hooks 會在以下寫入操作後自動執行驗證：

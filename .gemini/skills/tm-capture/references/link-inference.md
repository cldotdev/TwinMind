...
本文件定義建卡時的自動連結推理程序，以及連結建立所需的關係類型系統。由 `tm-capture` 的 Step 5.5 引用，main agent inline 執行（不使用 subagent）。
...
**Step 3 — 更新索引**（**單次 replace**）：以單次 replace tool invocation 完成以下所有變更：
...
old_string SHALL 涵蓋源 note entry、目標 note entry 和 stats 物件的連續 JSON 區塊；new_string 包含所有欄位更新後的完整版本。**當多條連結由 link-inference 接受時，所有連結的 Step 3 SHALL 合併為單次 replace**：old_string 涵蓋新卡片 entry、每張目標卡片 entry 和 stats 物件，new_string 包含所有受影響 entries 的完整更新版本（所有 links_to/linked_from 更新、所有 link_count 重算、stats.total_links 增加接受連結數、stats.last_updated 更新）。
...
6. **執行連結**（兩階段批次寫入）：
   - **Phase A — 卡片檔案編輯**：對每筆接受的連結，依序執行「建立連結程序」的 Step 1（寫入新卡片 `## Connections`）和 Step 2（寫入目標卡片反向連結）。此階段完成所有卡片 `.md` 檔案的 replace，不觸碰 vault-index.json
   - **Phase B — 單次索引更新**：所有卡片檔案編輯完成後，執行一次合併的 Step 3：single replace 涵蓋新卡片 entry、所有目標卡片 entries 和 stats 物件，new_string 包含所有接受連結的完整雙向更新（詳見 Step 3 說明）

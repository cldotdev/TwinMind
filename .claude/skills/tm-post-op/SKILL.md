...
**月度檔案自動建立：** 若 `changelog-YYYY-MM.md` 不存在，先以 Write tool 建立檔案，內容為 `# Changelog YYYY-MM` + 空行，再 append 條目。

**Append 方式：** 使用 Bash `echo >> "vault/System/changelog-YYYY-MM.md"` 或等效的尾部追加方式寫入條目。不使用 Read 或 Edit 讀取既有內容。
